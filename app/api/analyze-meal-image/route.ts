  ...
  try {
      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            { 
              type: "text", 
              text: "Analyze this meal and provide detailed nutritional information. Focus on accuracy for muscle gain tracking. Include portion sizes if visible." 
            },
            {
              type: "image_url",
              image_url: {
    ...
    const structuredCompletion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Convert the meal analysis into a structured JSON format with name, description, calories, protein, carbs, and fats fields. Ensure all numeric values are numbers, not strings.",
        },
        {
          role: "user",
          content: analysis || "",
